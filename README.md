...
### Limitations
1. Need to run the Python program `fix_quicken.py` prior to running the Sankey chart to make multiple fixes to the raw Quicken output (see below). This means you need to have Python3 installed and know how to run a Python program.
2. Reader initial default filename is hard-coded as "quicken_export-clean.xlsx" but can be changed using "Load another file" button (see Brief Instructions).
3. Reader explicilty uses only "Category" and "Amount" columns as defined in Quicken transaction export file. All other columns that are included in the Quicken transaction report are ignored but are made available in the raw data extract for debugging purposes.
4. Chart renders expenses transactions only. This potentially can cause noticable (but not massive) differences in Quicken reports that include reimbursement transactions. The Sankey diagram expenses amounts will be equal to or higher than the equivalent native Quicken report because of the lack of reimbursements. There is an enhancement issue on Github to fix this.
5. Another reason for small differences between Quicken reports and Sankey value is the use of rounded dollars in the Sankey diagram. 
6. The current rendering does not eliminate all flow/link visual overlaps. I gave up trying to completely eliminate them.
7. Data table sorting limited to only one column ascending/descending.
8. Changes to MIN_VIZ, font, and open file are not retained across sessions.
9. Read acknowledgements
...
3. Export the report as XLSX and save in the same directory as the index.html file. The default filename expected by the reader is quicken_export-clean.xlsx but you can name this file anything but must end in XLSX.
...
4. Run `fix_quicken.py -i <quicken_export.xlsx>` Substitute `quicken_export.xlsx` for whatever name you gave to the raw Quicken XLSX file. This program creates a new XLSX with the same filename with -clean appended (e.g., quicken_export.xlsx --> quicken_export-clean/xlsx). Use the "clean" version for the Sankey plot. The program does the following alterations to the raw Quicken output:

    * Removes the top header rows
    * Removes the bottom footer rows
    * Down-fills missing dates for split transactions (fixes GitHub issue #9) if a Date column is present
    8 Down-fills missing descriptions for split transactions if a Description column is present

5. If you wish to use a Python-based web server `webserver.py`, start the included Flask server in a separate Python virtual environment so the browser can fetch the XLSX:
...
If you do not have a file named quicken_export-clean.xlsx in the same directory as index.html, you will get this initial page. Use the "Load another file" in the upper left corner to use your XLSX.
