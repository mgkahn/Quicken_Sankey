...
1. Reader initial default filename is hard-coded as "quicken_export.xlsx" but can be changed using "Load another file" button (see Brief Instructions). 
...
The above changes have been incorporated into `index.html`. No manual manipulation of the raw Quicken XLSX file is required.

5. If you wish to use the provided Python-based web server `webserver.py`, start the Flask server in a separate Python virtual environment so the browser can fetch the XLSX:
...
- Rolling 36mo: Sets Mindate to 3 years earlier than MaxDate.
